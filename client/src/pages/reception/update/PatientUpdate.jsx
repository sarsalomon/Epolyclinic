...
import { getPatient, updatePatient } from '../../../http/userAPI';
import { ToastContainer, toast } from 'react-toastify';

const PatientUpdate = observer(() => {
    const {id} = useParams()
    ...
    useEffect(() => {
        ...
        if(id !== '' && id !== null && id !== undefined){
            getPatient(id).then(data => {
                setFish(data.fish)
                setPhone(data.phone)
                setBdate(data.dateOfBirth)
                setPassport(data.passport)
                setAddress(data.address)
                setSex(data.sex)
                setBloodGroup(data.bloodGroup)
                setMfy(data.NeighBorhoodId)
            })
        }
    }, [id])

    const Patientu = async () => {
        try{
            let data;
            ...
            data = await updatePatient(formData)
            toast.success(`Bemorni ma'lumotlari yangilandi`, {
    ...
    return (
        <div>
            <div className="application">
                <Helmet>
                    <meta charSet="utf-8" />
                    <title>Bemorni Yangilash</title>
                </Helmet>
            </div>
            <Container>
            <h2 className="text-center">Bemorni Yangilash</h2>
            <Form>
                    <Row>
                        <Col>
                    ...
                    <Row>
                        ...
                        <Col className="text-end">
                            <Button variant="success" onClick={Patientu} type="reset">Yangilash</Button>
                        </Col>
                    </Row>
                </Form>
            <ToastContainer />
